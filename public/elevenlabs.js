...
`);
	...
	*/
	const webScribeMicrophoneSetup = async (config, onAudioData) => {
		...
		try {
			...
			audioContext = new AudioContext(streamSampleRate ? { sampleRate: streamSampleRate } : {});
	...
	//#endregion
	//#region src/utils/uploadFile.ts
	const HTTPS_API_ORIGIN$1 = "https://api.elevenlabs.io";
	async function uploadFile({ conversationId, origin: rawOrigin, file, filename: filenameOverride }) {
		const origin = (rawOrigin ?? HTTPS_API_ORIGIN$1).replace(/^wss:\/\//, "https://").replace(/^ws:\/\//, "http://");
		const filename = filenameOverride ?? ("name" in file && typeof file.name === "string" ? file.name : `upload.${(file.type || "image/png").split("/").pop()?.split("+")[0]}`);
		const body = new FormData();
		body.append("file", file, filename);
		const response = await fetch(`${origin}/v1/convai/conversations/${conversationId}/files`, {
			method: "POST",
			body
		});
		if (!response.ok) {
	...
	var BaseConversation = class {
		...
		handlePing(event) {
			if (this.options.onPing) this.options.onPing(event.ping_event);
		}
		...
		handleAgentReasoningResponsePart(event) {
			if (this.options.onAgentReasoningResponsePart) this.options.onAgentReasoningResponsePart(event.reasoning_response_part);
		}
	...
	*/
	...
	function normalizeError(error) {
		if (typeof error === "object" && error !== null && "message_type" in error && "error" in error && typeof error.message_type === "string" && typeof error.error === "string") return error;
		return {
			message_type: "error",
			error: error instanceof Error ? error.message : String(error)
		};
		...
		setWebSocket(websocket) {
			...
			this.websocket.addEventListener("message", (event) => {
				try {
					...
					this._emitError(/* @__PURE__ */ new Error(`Failed to parse message: ${error}`));
				}
			});
			this.websocket.addEventListener("error", (error) => {
				if (this._closed) return;
				console.error("WebSocket error:", error);
				this._emitError(/* @__PURE__ */ new Error("WebSocket error"));
			});
	...
	*/
	var ScribeRealtime = class ScribeRealtime {
		...
		static buildWebSocketUri(options) {
			...
			if (options.includeLanguageDetection !== void 0) params.append("include_language_detection", options.includeLanguageDetection ? "true" : "false");
			...
			if (options.enableLogging !== void 0) params.append("enable_logging", options.enableLogging ? "true" : "false");
		...
		static async streamFromMicrophone(options, connection) {
			try {
				...
				if (connection._closed) {
					result.cleanup();
					connection._audioCleanup = void 0;
				}
			} catch (error) {
